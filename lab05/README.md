...
* **Step 2:** Create a new Java Class using the HelloWorld template in the lab's source code (**Examples/src/introsde/rest/servlet/HelloWorld.java**). The same is generated by Eclipse if you use *File -> New -> Servlet*
   * If you use Eclipse's wizard, make sure that you later modify WebServlet Annotation so that it defines a name and urlPatterns - e.g., *@WebServlet(name = "ZYZServlet", urlPatterns = "/helloworld")*. Sometimes the service does not work otherwise.
