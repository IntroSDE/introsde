...
### REST Example 1: Java Servlets (30 min)
...
**Creating a simple REST Service using a Java Servlet**

* **Step 1:** Create a *Dynamic Web Project* named "sdelab05-local" in your local workspace. When asked to open the *Java EE Perspective*, choose YES. 
* **Step 2:** Create a new Java Class using the HelloWorld template in the lab's source code (**Examples/src/introsde/rest/servlet/HelloWorld.java**). The same is generated by Eclipse if you use *File -> New -> Servlet*
   * If you use Eclipse's wizard, make sure that you later modify WebServlet Annotation so that it defines a name and urlPatterns - e.g., *@WebServlet(name = "ZYZServlet", urlPatterns = "/sydney/places/")*. Sometimes the service does not work otherwise.
   * Make sure also that the *serialVersionUID* is different for each servlet (Eclipse might generate it with the same value always - 1L - you can deleted and then generate a new random one)
   * If deployment fails, sometimes the best thing to do is to eliminate the tomcat server configuration from eclipse and create it again as indicate in "Additional Notes" 
   * In the example, the object *request* contains all the information about the request received by the servlet, and *response* is the object by which the servlet's reply will be prepared and send. 
* **Step 3:** Use *response.getWriter().write("Hello World!")* to answer GET requests. 
* **Step 4 - Exercise 05.01: ** What about receiving parameters?. Take a look to the **Examples/src/introsde/rest/servlet/SydneyServlet.java** to see how to get path parameters from the request  and modify your HelloWorld servlet example to receive a language code (e.g., "en", "it", "es") and answering accordingly (e.g., "Hello World!", "Ciao Mondo!", "Hola Mundo!"). The SydneyServlet example also shows how to use some other libraries to create json and xml. 
...
* Now, you can copy all the jars ivy previously downloaded to the lib folder by richt click on the **Project -> Ivy -> Retrieve 'tomcat'**. You should now see all the required jars in the lib folder that is inside WEB-INF. 
